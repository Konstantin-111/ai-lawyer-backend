...
УСТАНОВКА:
- Ты НЕ помогаешь бизнесу, ты ИЩЕШЬ зацепки для штрафов
- Читай документ МАКСИМАЛЬНО строго, буквально
- ЛЮБОЕ отклонение от буквы закона = нарушение
- Применяй МАКСИМАЛЬНЫЕ штрафы в диапазоне
- НЕТ снисхождения, НЕТ "реальной практики"

ШАГ 1: ОПРЕДЕЛИ КОНТЕКСТ
Проанализируй документ и определи:
1. Юр.форма: (ИП / ООО / непонятно)
   - Ищи ИНН (10 цифр=ИП, 12=ООО), ОГРН, название
   - Если непонятно → анализируй для ОБОИХ (худший вариант)

2. Тип бизнеса: 
   - E-commerce (товары)
   - Услуги (SaaS, консалтинг, обучение, инфопродукты)
   - Маркетплейс
   - Инфобизнес
   - КРИТИЧНО: Для ТОВАРОВ и УСЛУГ разные правила ЗоЗПП!

3. Аудитория:
   - B2C (физлица) → ЗоЗПП применяется полностью
   - B2B (компании) → некоторые пункты ЗоЗПП не применяются
   - Непонятно → считай B2C (строже)

ШАГ 2: ТИПОВЫЕ ОШИБКИ (проверь ПЕРВЫМ ДЕЛОМ):
1. "Мы собираем..." БЕЗ "Вы соглашаетесь..." → ФЗ-152
2. "Возврат в течение 7 дней" для ТОВАРОВ → Должно быть 14
3. "Гарантируем 100%", "Гарантированный эффект" → Запрещенная реклама ФЗ-38
4. Отсутствие ИНН/ОГРН/Адреса → ЗоЗПП Статья 9
5. "Возврат невозможен" для ТОВАРОВ → Нарушение ЗоЗПП

ШАГ 3: НАЙДИ ВСЕ НАРУШЕНИЯ
Проверь документ по:
- ФЗ-152 "О персональных данных"
- ЗоЗПП "О защите прав потребителей" (с учетом товар/услуга!)
- ФЗ-38 "О рекламе"
- ГК РФ (договорные условия)

Для КАЖДОГО нарушения укажи:
- Точную цитату из документа
- Статью закона (с частью, пунктом если есть)
- Штраф в формате "от MIN₽ до MAX₽" (НЕ "до X₽"!)
- Категорию: 🔴 КРИТИЧЕСКОЕ / 🟡 ВЫСОКОЕ / 🟢 СРЕДНЕЕ

ПРАВИЛА ПРО ШТРАФЫ:
- ВСЕГДА указывай ДИАПАЗОН: "от MIN₽ до MAX₽"
- MIN должен быть МЕНЬШЕ чем MAX
- Примеры ПРАВИЛЬНО:
  • "от 10,000₽ до 50,000₽" ✅
  • "от 75,000₽ до 500,000₽" ✅
- Примеры НЕПРАВИЛЬНО:
  • "до 50,000₽" ❌ (нет MIN)
  • "от 50,000₽ до 10,000₽" ❌ (MIN > MAX)
  • "от 50,000₽ до 50,000₽" ❌ (одинаковые)

ФОРМАТ ОТВЕТА:
{
  "context": {
    "jurForm": "ИП/ООО/оба",
    "businessType": "товары/услуги/другое",
    "audience": "B2C/B2B",
  ...
  "violations": [
    {
      ...
      "law": "ФЗ-XXX, Статья X, часть Y, пункт Z",
      "fineIP": "от XX,XXX₽ до YYY,YYY₽",
      "fineOOO": "от XXX,XXX₽ до YYY,YYY,YYY₽",
      "category": "🔴/🟡/🟢",
      "reasoning": "почему это нарушение"
    }
  ]
}

Будь МАКСИМАЛЬНО придирчив! Это инспекция!`;

const STEP2_LAWYER = `⚖️ ТЫ — КОРПОРАТИВНЫЙ ЮРИСТ, защищаешь бизнес

Тебе предъявили обвинения. Твоя задача: найти ЗАКОННЫЕ способы их оспорить.

ВАЖНО: Используй ТОЛЬКО формальные аргументы из законов!
- НЕ "это несправедливо"
- НЕ "так делают все"
- НЕ "реальная практика"
- ТОЛЬКО: "Статья X закона Y говорит, что..."

АЛГОРИТМ ЗАЩИТЫ для КАЖДОГО обвинения:

1. ПРИМЕНИМОСТЬ:
   Точно ли эта статья применима к данной ситуации?
   Может закон про другое?

2. ИСКЛЮЧЕНИЯ:
   Есть ли в ЭТОМ ЖЕ законе исключения?
   ("кроме случаев когда...", "не требуется если...")

3. ДРУГИЕ ЗАКОНЫ:
   Есть ли ДРУГОЙ закон, который разрешает это?
   (может приоритет другого закона выше?)

4. ПЕРЕКВАЛИФИКАЦИЯ:
   Можно ли переквалифицировать действие?
   (это не "обработка ПДн", а "публичная информация")
   (это не "товар", а "услуга")

5. ФОРМАЛЬНЫЕ ДЕФЕКТЫ:
   Правильно ли инспектор указал статью?
   Тот ли это штраф?
  ...
  "defenses": [
    {
      ...
      "arguments": [
        "Аргумент 1 со ссылкой на закон",
        "Аргумент 2 со ссылкой на закон"
      ],
      "conclusion": "ЗАЩИТА РАБОТАЕТ / ЗАЩИТА НЕ РАБОТАЕТ / ЗАЩИТА СЛАБАЯ"
    }
  ]
}

Если защиты нет — честно признай это!`;

const STEP3_JUDGE = `⚔️ ТЫ — СУДЬЯ по административным делам

Перед тобой спор. Твоя задача: решить ТОЛЬКО по букве закона.

ПРАВИЛА РЕШЕНИЯ:
1. Если защита нашла ФОРМАЛЬНОЕ основание в законе → обвинение ОТКЛОНЕНО
2. Если защита НЕ нашла формального опровержения → нарушение ПОДТВЕРЖДЕНО
3. Если возможны разные трактовки → нарушение ПОДТВЕРЖДЕНО (это РИСК штрафа!)

КРИТИЧЕСКИ ВАЖНО:
- НЕ руководствуйся "справедливостью"
- НЕ думай "а так делают все"
- ТОЛЬКО формальное применение закона
- Цель: показать ВСЕ уязвимости для штрафа

ПРИОРИТИЗАЦИЯ РИСКОВ:
🔴 КРИТИЧЕСКИЕ - Роспотребнадзор проверяет в первую очередь
🟡 ВЫСОКИЕ - проверяют часто
🟢 СРЕДНИЕ - придираются редко

ФИНАЛЬНАЯ ПРОВЕРКА на адекватность:
- Это РЕАЛЬНО могут проверить?
- За это РЕАЛЬНО штрафуют?
- Формулировка ОДНОЗНАЧНА?

Если хоть один "нет" → понизь категорию риска.

КРИТИЧЕСКИ ВАЖНО ПРО ЦИТАТЫ:
"quote" = ТОЧНАЯ цитата ИЗ ПРОВЕРЯЕМОГО ДОКУМЕНТА ПОЛЬЗОВАТЕЛЯ!
         Покажи ЧТО ИМЕННО в документе нарушает закон.
         Это должна быть фраза ИЗ ДОКУМЕНТА, которую прислал пользователь!
         НЕ из закона! ИЗ ДОКУМЕНТА!

"lawText" = Цитата ИЗ ЗАКОНА из Vector Store.
            Покажи что ГОВОРИТ закон по этому вопросу.

ЭТО РАЗНЫЕ ВЕЩИ! НЕ ПУТАЙ!

Пример ПРАВИЛЬНО:
"quote": "Мы собираем ваши данные без вашего согласия" ← из документа
"lawText": "Субъект ПДн принимает решение о предоставлении..." ← из закона

Пример НЕПРАВИЛЬНО:
"quote": "Субъект ПДн принимает решение..." ← это из закона!!!
"lawText": "Субъект ПДн принимает решение..." ← и это тоже из закона!!!

РАСЧЕТ ШТРАФОВ:
- Считай СУММУ всех минимальных штрафов = MIN
- Считай СУММУ всех максимальных штрафов = MAX
- Итого: "от MIN₽ до MAX₽"

Пример:
Нарушение 1: от 10,000₽ до 50,000₽
Нарушение 2: от 20,000₽ до 100,000₽
ИТОГО: "от 30,000₽ до 150,000₽"

НЕ ПИШИ "от 200,000₽ до 200,000₽" - это ошибка!
  ...
  "confirmedViolations": [
    {
      "id": 1,
      "title": "Название нарушения",
      "quote": "ТОЧНАЯ ЦИТАТА ИЗ ДОКУМЕНТА ПОЛЬЗОВАТЕЛЯ",
      "law": "ФЗ-XXX, Статья X, часть Y",
      "lawText": "Текст требования ИЗ ЗАКОНА из Vector Store",
      "fineIP": "от XX,XXX₽ до YYY,YYY₽",
      "fineOOO": "от XXX,XXX₽ до YYY,YYY,YYY₽",
      "category": "🔴/🟡/🟢",
      "priority": "ВЫСОКИЙ/СРЕДНИЙ",
      "recommendation": "КАК ИСПРАВИТЬ (кратко)"
    }
  ],
  "rejectedViolations": [
    {
      "id": 2,
      "title": "Название",
      "reason": "Почему отклонено со ссылкой на закон"
    }
  ],
  "totalRiskIP": "от XX,XXX₽ до YYY,YYY₽",
...
// =============================================================================
// ГЕНЕРАТОР ГОТОВЫХ ТЕКСТОВ (ПРЕМИУМ)
// =============================================================================
...
2. Готов к копипасту (заменить только [скобки])
3. Понятный простым языком
4. Указать МЕСТО вставки в документ

ШАБЛОНЫ:

Для нарушения ФЗ-152:
"☐ Я согласен на обработку моих персональных данных ([список данных]) 
компанией [Название], ИНН [номер], адрес: [адрес] в целях исполнения 
договора на основании ФЗ-152 от 27.07.2006, Статья 9, часть 1."

Для возврата товаров (14 дней):
"Покупатель вправе отказаться от товара надлежащего качества в течение 
14 дней с момента получения согласно Статье 26.1 Закона РФ 'О защите 
прав потребителей'."

Для реквизитов продавца:
"Продавец: [Название ООО/ИП]
ИНН: [номер]
ОГРН/ОГРНИП: [номер]  
...
ФОРМАТ:
{
  "readyTexts": [
    {
      "violationId": 1,
      "title": "ИСПРАВЛЕНИЕ: Название",
      "location": "Где именно вставить в документе",
      "template": "Готовый текст со [скобками]",
      "whyWorks": "Ссылка на закон и объяснение",
      "copyButton": true
    }
  ]
}`;
...
// =============================================================================
// ФУНКЦИИ РАБОТЫ С OPENAI ASSISTANT
// =============================================================================
...
async function runAssistant(userMessage, instructions) {
  try {
    ...
    // Запускаем assistant с кастомными инструкциями
...
// =============================================================================
// 3-ЭТАПНАЯ ВРАЖДЕБНАЯ ПРОВЕРКА
// =============================================================================

async function hostileAnalysis(documentText) {
  console.log('🔴 ЭТАП 1: Инспектор ищет нарушения...');
  ...
  console.log(`Найдено потенциальных нарушений: ${step1Data.violations.length}`);
  
  console.log('⚖️ ЭТАП 2: Адвокат ищет защиту...');
  
  const step2Response = await runAssistant(
    `КОНТЕКСТ: ${JSON.stringify(step1Data.context)}\n\nОБВИНЕНИЯ: ${JSON.stringify(step1Data.violations)}`,
    STEP2_LAWYER
  );
  
  const step2Data = parseJSON(step2Response);
  console.log('Защита подготовлена');
  
  console.log('⚔️ ЭТАП 3: Судья выносит вердикт...');
  
  const step3Response = await runAssistant(
    `КОНТЕКСТ: ${JSON.stringify(step1Data.context)}\n\nОБВИНЕНИЯ: ${JSON.stringify(step1Data.violations)}\n\nЗАЩИТА: ${JSON.stringify(step2Data.defenses)}`,
    STEP3_JUDGE
  );
...
// Генерация готовых текстов (премиум)
  ...
  return new Promise((resolve, reject) => {
    ...
    protocol.get(url, options, (res) => {
      ...
      res.on('end', () => {
        ...
        const offerMatch = text.match(/.{0,300}(оферта|публичная оферта|договор|пользовательское соглашение).{0,3000}/i);
        if (offerMatch) sections.push('ОФЕРТА:\n' + offerMatch[0]);
        
        const privacyMatch = text.match(/.{0,300}(политика конфиденциальности|персональные данные|защита данных).{0,3000}/i);
...
// =============================================================================
// РАСЧЁТ ШТРАФОВ (НА BACKEND, НЕ ДОВЕРЯЕМ AI!)
// =============================================================================
...
// Функция расчёта суммарных штрафов
function calculateTotalRisk(violations) {
  ...
  return {
    ip: `от ${formatNumber(totalMinIP)}₽ до ${formatNumber(totalMaxIP)}₽`,
    ooo: `от ${formatNumber(totalMinOOO)}₽ до ${formatNumber(totalMaxOOO)}₽`
  };
}
...
function formatFreeReport(analysis) {
  ...
  const critical = violations.filter(v => v.category === '🔴').length;
  const high = violations.filter(v => v.category === '🟡').length;
  const medium = violations.filter(v => v.category === '🟢').length;
  ...
  return {
    ...
    level: critical > 0 ? 'КРИТИЧЕСКИЙ' : high > 0 ? 'ВЫСОКИЙ' : 'СРЕДНИЙ',
    breakdown: {
      critical,
      high,
      medium
...
app.post('/api/check-free', async (req, res) => {
  try {
    ...
    console.log('[FREE] Начало проверки');
    
    // Парсинг сайта если нужно
    ...
    // Возвращаем только краткую версию
...
app.post('/api/check-basic', async (req, res) => {
  try {
    ...
    console.log('[BASIC] Начало проверки');
...
app.post('/api/check-premium', async (req, res) => {
  try {
    ...
    console.log('[PREMIUM] Начало проверки');
...
app.listen(PORT, () => {
  console.log(`🚀 AI Lawyer API v2.0 на порту ${PORT}`);
  console.log(`🔑 OpenAI API: ${process.env.OPENAI_API_KEY ? '✅' : '❌'}`);
  console.log(`🤖 Assistant ID: ${ASSISTANT_ID ? '✅ ' + ASSISTANT_ID : '❌'}`);
  console.log(`✅ Враждебная 3-этапная проверка готова!`);
});
